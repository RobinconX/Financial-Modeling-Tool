...
describe('targetCompoundValue', () => {
  ...
  it('year after the anchor compounds then adds scheduled deposits; perpetual only after last projection', () => {
    const p = basePortfolio({
      deposits: [
        newOpeningDeposit(10_000, 2026),
    ...
    const lastStated = 2028
    // 2026: 10000 (anchor)
    // 2027: 10000 * 1.1 + 2000 = 13000 (no perpetual while projections remain)
    // 2028: 13000 * 1.1 + 500 = 14800
    // 2029: 14800 * 1.1 + 1000 perpetual = 17280
    expect(
      targetCompoundValue(10_000, 10, 2026, 2026, { isNow: false, year: 2026 }, p, lastStated),
    ).toBe(10_000)
    expect(
      targetCompoundValue(10_000, 10, 2026, 2026, { isNow: false, year: 2027 }, p, lastStated),
    ).toBeCloseTo(13_000, 6)
    expect(
      targetCompoundValue(10_000, 10, 2026, 2026, { isNow: false, year: 2028 }, p, lastStated),
    ).toBeCloseTo(14_800, 6)
    expect(
      targetCompoundValue(10_000, 10, 2026, 2026, { isNow: false, year: 2029 }, p, lastStated),
